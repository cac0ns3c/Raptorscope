...
- [x] **DONE (C1)** — streaming seam (`stream_text` + `agentic_stream`), SSE `/ai/summary/stream` and `/ai/copilot/stream` endpoints, incremental UI (Overview summary; Copilot renders the live tool-call trace then streams the verdict). Live-validated against real Claude. (Triage streaming trivially follows the same seam if wanted.)
...
- [x] **DONE (D1)** — role claims in the signed token (viewer/analyst/admin; `RAPTORSCOPE_AUTH_ROLES`), analyst-gate on AI + `/hunt`, append-only `raptorscope.audit` log, and a TLS reverse-proxy overlay (`docker-compose.tls.yml` + `deploy/tls/`, `make certs`, HSTS + SSE pass-through).
...
- [x] **DONE (E1)** — CI `integration` job (live ES ingest smoke + ES-native parity) **and** a Playwright `e2e` job driving the built SPA against the offline sample (case picker, fleet hunt, tab navigation, search). Config + specs typecheck and `playwright test --list` discovers all 3; the browser run executes on CI Linux (Node-26/Chromium can't launch in this sandbox).
